...
def _crossfit(models: Union[ModelSelector, List[ModelSelector]], folds, use_ray, ray_remote_fun_option,
    ...
    for model in models:
        ...
        if folds is None:  # skip crossfitting
            ...
            for idx, (train_idxs, test_idxs) in enumerate(folds):
                if use_ray:
                    nuisance_temp, model, score_temp = ray.get(fold_refs[idx])
                else:
                    nuisance_temp, model, score_temp = _fit_fold(model, train_idxs, test_idxs,
                                                                 calculate_scores, args, kwargs)

                if idx == 0:
                    nuisances = tuple([np.full((args[0].shape[0],) + nuis.shape[1:], np.nan)
...
class _OrthoLearner(TreatmentExpansionMixin, LinearCateEstimator):
    ...
    def score(self, Y, T, X=None, W=None, Z=None, sample_weight=None, groups=None):
        ...
        if expand_list:
            ...
            n_splits = len(self._models_nuisance[0][1])
